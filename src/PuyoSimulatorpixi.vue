  ...
  export default {
    ...
    methods: {
      ...
      async step(){
        ...
        this.playable = false;
        condition = await this.gravity();
        if(!condition){
          condition = await this.clear();
        }
      ...
      async play(){
        ...
        while(condition){
          condition = await this.gravity();
          condition = condition || await this.clear();
        }
        this.playable = condition;
      },
      async gravity(){
        ...
        for (let diff of gravitationalDiff){
          ...
          tl.to(
            ...
            (20+4*(diff.positional.row - diff.otherPositional.row))/60, 
            {pixi:{y:(this.field.dimension.rows - diff.otherPositional.row - 1) * 32},
             ease:"Linear"}
          );
          tl.to(
            this.pixifield[diff.positional.row][diff.positional.column],
        ...
        // map async functions
        const drop_animation = tl_arr.map(async (obj) => { return this.timelinePromise(obj[0],obj[1],"gravity"); });
      ...
      loadbg(){
        ...
        pole1_left.y = this.field.dimension.hiddenRows*32;
        ...
        pole1_right.x = this.app.screen.width - 32;
        pole1_right.y = this.field.dimension.hiddenRows * 32;
        this.app.stage.addChild(pole1_right);
        let pole0_left = new PIXI.Sprite(bgtex["pole0"]);
        pole0_left.y = this.field.dimension.hiddenRows * 32;
        this.app.stage.addChild(pole0_left);
        let pole0_right = new PIXI.Sprite(bgtex["pole0"]);
        pole0_right.x = this.app.screen.width - 32;
        pole0_right.y = this.field.dimension.hiddenRows * 32;
        this.app.stage.addChild(pole0_right);
        let block = new PIXI.extras.TilingSprite(
            bgtex['block'],
            this.field.dimension.columns*32,
            32
        );
        block.x = 32;
        block.y = this.app.screen.height - 32;
      ...
      setpuyo(obj,row,col,connections){
        ...
        switch(obj) {
          case Field.Object.RED:
            this.pixifield[row][col].texture = skintex[`red${connections}`];
            this.pixifield[row][col].x = 32 + col * 32;
            this.pixifield[row][col].y = (this.field.dimension.rows - row - 1) * 32;
            this.app.stage.addChild(this.pixifield[row][col]);
            break;
          case Field.Object.GREEN:
            this.pixifield[row][col].texture = skintex[`green${connections}`];
            this.pixifield[row][col].x = 32 + col * 32;
            this.pixifield[row][col].y = (this.field.dimension.rows - row - 1) * 32;
            this.app.stage.addChild(this.pixifield[row][col]);
            break;
          case Field.Object.BLUE:
            this.pixifield[row][col].texture = skintex[`blue${connections}`];
            this.pixifield[row][col].x = 32 + col * 32;
            this.pixifield[row][col].y = (this.field.dimension.rows - row - 1) * 32;
            this.app.stage.addChild(this.pixifield[row][col]);
            break;
          case Field.Object.YELLOW:
            this.pixifield[row][col].texture = skintex[`yellow${connections}`];
            this.pixifield[row][col].x = 32 + col * 32;
            this.pixifield[row][col].y = (this.field.dimension.rows - row - 1) * 32;
            this.app.stage.addChild(this.pixifield[row][col]);
            break;
          case Field.Object.PURPLE:
            this.pixifield[row][col].texture = skintex[`purple${connections}`];
            this.pixifield[row][col].x = 32 + col * 32;
            this.pixifield[row][col].y = (this.field.dimension.rows - row - 1) * 32;
            this.app.stage.addChild(this.pixifield[row][col]);
            break;
    ...
    created(){
      ...
      this.app = new PIXI.Application({
        width: (this.field.dimension.columns + 2)*32,
        height: (this.field.dimension.rows + 1)*32,
        antialias: true,
        transparent: false,
        resolution: 1
      });
